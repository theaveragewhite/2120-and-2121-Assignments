...
Node *insert_random(Node *T, int v, int r)
{
  ...
  else
  {
    ...
    // If random number between 0 and T's size (inclusive) is 0, insert at the root
  int totalSize = T->size + 1; // Increment size for the new node
  if (rand() % totalSize == 0)
  {
  ...
  {
    //If r is less than or equal to the size of the left subtree, insert left
    T->left = insert_random(T->left, v, r);
  }
  else
  {
    //Otherwise, insert right
    T->right = insert_random(T->right, v, r - leftSize - 1);
  }
